 ...
 */
package com.byteera.hxlib.model;
...
public class HXNotifier {
    ...
    public synchronized void onNewMsg(EMMessage message) {
        LogUtil.d("Receive new msg: %s", message.toString());
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try
        {
            LogUtil.d("Got notification message, msg: %s, isForeground: %s", message, isForeground);
            String contentTitle;
            String notifyText = "";
            ...
            if (message.getType() != EMMessage.Type.CMD) {
                ...
                User user = DBManager.getInstance(appContext).getUserByEasemobId(easemobId);
                if(user != null)
                {
                    notifyText = user.getNickName() + " ";
                }
            ...
            else
            {
                ...
                if(cmdMsgBody.action.equals("addfriendrequest"))
                {
                    String srcuser = message.getStringAttribute("srcuser", "");
                    User user = DBManager.getInstance(appContext).getUserByUserId(srcuser);
                    if(user != null)
                    {
                        srcuser = user.getNickName();
                    }
                    notifyText = srcuser + "请求加您为好友。";
                }

                if(cmdMsgBody.action.equals("addfriendresult"))
                {
                    String destUser = message.getStringAttribute("dstuser", "");
                    User user = DBManager.getInstance(appContext).getUserByUserId(destUser);
                    if(user!=null)
                    {
                        destUser = user.getNickName();
                    }
                    notifyText = destUser + "已经通过了您的好友请求。";
                }

                if(cmdMsgBody.action.equals("bulletin"))
                {
                    notifyText = "收到新的通知，请查阅。";
                }
            }

            // create and sendAsync notificaiton
        ...
        try {
            ...
            if (model.getSettingMsgSound()) {
                if (ringtone == null) {
                    ...
                    if (ringtone == null) {
                        LogUtil.d("cant find ringtone at:" + notificationUri.getPath());
                        return;
                    }
                }
