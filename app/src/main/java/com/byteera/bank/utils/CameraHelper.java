    ...
    public void startCapture(){
        try {
            ...
            if (mCamera == null) {
                ...
                LogUtil.e("camera count:" + camera_count);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    for (int i = 0; i < camera_count; i++) {
                        ...
                        if (info.facing == CameraInfo.CAMERA_FACING_FRONT) {
                            LogUtil.e("to open front camera");
            ...
            LogUtil.e("pzy bitsperpixel: " + bitsperpixel);
...
//            yuv_Rotate90lr = new byte[mwidth * mheight * bitsperpixel / 8];
            ...
            LogUtil.d( "camera start preview");
        } catch (Exception e) {
            e.printStackTrace();
            if(mCamera != null)
                mCamera.release();
    ...
    boolean isScreenOriatationPortrait() {
        return MyApp.applicationContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT;
    }
}
